...
export default function Notificacoes() {
  ...
  const checarAtualizacoesBackend = useCallback(async () => {
    ...
    const { keyNotificacoes, keyHistoricoIds } =
      getStorageKeys(usuario.id);
    ...
    try {
      ...
      const notificacoesSalvas =
        JSON.parse(localStorage.getItem(keyNotificacoes)) || [];

      const historicoIds =
        JSON.parse(localStorage.getItem(keyHistoricoIds)) || [];
      ...
      pedidos.forEach((pedido) => {
        ...
        if (!jaProcessado) {
          const valorFormatado = pedido.total
            ? Number(pedido.total)
                .toFixed(2)
                .replace(".", ",")
            : "0,00";
  ...
  const gerarNotificacaoIA =
    useCallback(async () => {
      ...
      const criarPrompt = (tema) =>
        `Crie uma notificação curta e empolgante para um e-commerce de futebol chamado SoccerGear. 
O cliente se chama ${nomeUsuario}. O tema é: ${tema}.
Responda APENAS um JSON válido neste formato: {"titulo": "...", "descricao": "..."}`;

      const gerarUnica = async (
        tema
      ) => {
        const model =
      ...
      try {
        if (!lastTime) {
          const temasIniciais = [
            ...
            "lançamento de chuteira",
          ];

          const promises =
            temasIniciais.map((tema) =>
              gerarUnica(tema)
            );
    ...
    <main className="notificacoes-container">
      <div className="notificacoes-header">
        ...
        <div className="acoes-header">
          ...
          <button
            className="btn-acao limpar"
            onClick={
              abrirModalLimparTudo
            }
          >
            Limpar tudo
          </button>
        ...
        >
          ...
          >
            ...
            <div className="modal-body">
              <span
                className={`badge ${notificacaoSelecionada.categoria}`}
              >
                {
                  notificacaoSelecionada.categoria
                }
              </span>

              <p className="data-detalhe">
                {
                  notificacaoSelecionada.dataFormatada
                }
              </p>

              <hr />

              <p className="texto-completo">
                {
